...
function extractGeometryFromLayout(
  ...
  console.log('[🎯COORD] ELK-LAYOUT - START:', {
    scopeId,
  ...
  function extractFromNode(node: ElkGraphNode, isGroup: boolean, parentElkPos: { x: number; y: number } = { x: 0, y: 0 }): void {
    ...
    if (isGroup && (parentElkPos.x !== 0 || parentElkPos.y !== 0)) {
      console.log('[🎯COORD] ELK-LAYOUT - nested group conversion:', {
        nodeId: id,
    ...
    if (isGroup) {
      ...
      console.log('[🎯COORD] ELK-LAYOUT - WRITING group to ViewState:', {
        nodeId: id,
        isScopeRootChild,
        isNested,
        elkRelative: `${elkRelativePos.x},${elkRelativePos.y}`,
        ...
        problem2_check: isNested && Math.abs(absolutePos.x - parentAbsolute.x) < 1 && Math.abs(absolutePos.y - parentAbsolute.y) < 1
          ? '⚠️ PROBLEM 2: Nested group absolute equals parent absolute - ELK relative not added!'
          : 'OK',
      });
    } else {
    ...
    if (node.children) {
      console.log('[🎯COORD] ELK-LAYOUT - processing nested children:', {
        parentId: id,
      ...
      node.children.forEach(child => {
        ...
        console.log('[🎯COORD] ELK-LAYOUT - recursing to nested child:', {
          parentId: id,
          parentAbsolute: `${absolutePos.x},${absolutePos.y}`,
          childId: child.id,
          childIsGroup,
  ...
  if (layout.children) {
    console.log('[🎯COORD] ELK-LAYOUT - extracting scope root children:', {
      scopeId,
    ...
    layout.children.forEach(child => {
      const childIsGroup = !!(child.children && child.children.length > 0);
      console.log('[🎯COORD] ELK-LAYOUT - extracting scope root child:', {
        scopeId,
        childId: child.id,
        childIsGroup,
        childElkRelative: `${child.x || 0},${child.y || 0}`,
