...
export function findContainingGroup(
  ...
  if (!viewStatePos) {
    console.log('[🎯COORD] findContainingGroup - MISSING ViewState:', { nodeId: node.id });
  }

  const nodeBounds = getNodeBounds(node, allNodes, viewState, useReactFlowPosition);
  if (!nodeBounds) {
    console.log('[🎯COORD] findContainingGroup - No node bounds:', { nodeId: node.id });
    return null;
  }
  ...
  for (const group of groups) {
    ...
    if (isFullyContained) {
      console.log('[🎯COORD] FOUND containing group:', {
        nodeId: node.id,
        groupId: group.id,
