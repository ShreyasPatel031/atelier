...
> IMPORTANT ENGINEERING RULE FOR CANVAS
>
> Do not add new logic or components directly to `client/components/ui/InteractiveCanvas.tsx`.
> Keep it as an orchestrator. Add new interactions/helpers in separate modules
> (e.g., `client/components/ui/canvasInteractions.ts`) and import them.
> This keeps the canvas maintainable and prevents performance regressions.
